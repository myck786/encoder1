...
async def encode(event, msg, scale=0):
    ...
    cmd = ''
    if scale == 240:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset ultrafast -s 426x240 -crf 18 -c:a aac -c:s copy """{out}""" -y'
    elif scale == 360:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset ultrafast -s 640x360 -crf 20 -c:a aac -c:s copy """{out}""" -y'
    elif scale == 480:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset ultrafast -s 854x480 -crf 23 -c:a aac -c:s copy """{out}""" -y'
    elif scale == 720:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset ultrafast -c:a aac -c:s copy """{out}""" -y'
    try:
